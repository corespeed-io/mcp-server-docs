...
Runs directly on your machine for development; requires API key.

**Installation:** `npx @coingecko/coingecko-mcp`

## Configuration

### Claude Desktop (Free Users)
...
In Claude.ai, select "Add connectors" -> "Add custom connector" and enter the MCP server URL.

### ChatGPT Integration

Enable Developer Mode in settings, then create a new connector with the MCP server endpoint.

### Local Setup with API Key (Pro API)
